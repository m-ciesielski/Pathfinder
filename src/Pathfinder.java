...
class GraphCreatorModel{
	...
	public int getNodeIndex(Node arg){return nodeContainer.indexOf(arg);}
	public void setPathStart(Node pathStart){this.pathStart=pathStart;}
	public void setPathEnd(Node pathEnd){this.pathEnd=pathEnd;}
...
class Graph {
	...
	*/
	private void pathfindingMechanics (Vector<Path> pathContainer, Path basePath, int pathLength, Node end)
	{
		...
		if(basePath.size()==pathLength-1){
			int index=pathContainer.indexOf(basePath);
			boolean correctPath=false;
			for(int i=0;i<basePath.lastElement().getNeighboursCount();++i)
			{
				if(basePath.lastElement().getNeighbour(i).equals(end)){
					correctPath=true;
					basePath.add(end);
					if(index<pathContainer.size()-1){
		...
		else
		{
			...
			for(int i=0;i<basePath.lastElement().getNeighboursCount();++i)
			{
				if(basePath.checkNode(basePath.lastElement().getNeighbour(i),end)){
					++possibleMoves;
				}
			}
			...
			//gdy pMoves>1 dodanie nowych sciezek do kontenera pathContainer)
				...
				for(int i=0;i<basePath.lastElement().getNeighboursCount();++i)
				{
					if(basePath.checkNode(basePath.lastElement().getNeighbour(i),end)==true)
					{
						if(makedMoves==possibleMoves-1){
							basePath.add(basePath.lastElement().getNeighbour(i));
							break;
						}
						else{
							pathContainer.addElement(new Path (basePath));
							pathContainer.lastElement().add(basePath.lastElement().getNeighbour(i));
							++makedMoves;
						}
					}
...
class Path
{
	...
	public Path (){vector=new Vector <Node> ();}
	...
	public void add(Node arg){vector.add(arg);}
	public Node get(int index){return vector.get(index);}
	public Node lastElement(){return vector.lastElement();}
